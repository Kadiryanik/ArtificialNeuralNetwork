...
AnnMatrix^ AnnMatrix::fnet(int f){
	int i, j;
	...
	case 1: // sigmoid
		for (i = 0; i < this->row; i++){
			for (j = 0; j < this->column; j++){
				this->matrix[i * this->column + j] = (2 / (1 + exp(-this->matrix[i * this->column + j]))) - 1;
			}
		}
		break;
	case 2: // sigmoid
		for (i = 0; i < this->row; i++){
			for (j = 0; j < this->column; j++){
				this->matrix[i * this->column + j] = (2 / (1 + exp(-this->matrix[i * this->column + j] * 0.000001))) - 1;
			}
		}
		break;
	default:
...
AnnMatrix^ AnnMatrix::operator*(const AnnMatrix^ other){
	...
	if (this->row == other->row && this->column == other->column){
		AnnMatrix^ temp = gcnew AnnMatrix(this->row, this->column);

		for (i = 0; i < this->row; i++){
			for (j = 0; j < this->column; j++){
				temp->matrix[i * this->column + j] = this->matrix[i * this->column + j] * other->matrix[i * this->column + j];
			}
		}
		return temp;
	}
