				 ...
				 while (!isDone || E > EPSILON){
					 ...
					 if (classOneSize >= classTwoSize){
						 ...
						 for (i = 0; i < classOneSize; i++){
							 ...
							 double fTnet;
							 double dMinusO = CLASS_ONE_D - fNet(radioButtonFnet1->Checked, net, &fTnet);
							 ...
							 if (j < classTwoSize){
								 ...
								 double net = netMatrix->getMatrix()[0];
								 double fTnet;
								 double dMinusO = CLASS_TWO_D - fNet(radioButtonFnet1->Checked, net, &fTnet);
								 if (dMinusO > EPSILON || dMinusO < -EPSILON){
									 E += (0.5 * dMinusO * dMinusO);
									 w += (columnVector * (c * dMinusO * fTnet));
									 isDone = 0;
					 ...
					 if ((counter % PRINT_FREQ) == 0){
						 ...
						 int px1 = 0;
						 int py1 = ((px1 - PICTURE_BOX_SIZE_DIV_TWO) * matrixHolder[0] + PICTURE_BOX_SIZE_DIV_TWO * matrixHolder[1] + matrixHolder[2] * pad) / (matrixHolder[1]);
						 refAnnPoints->addPoint(px1, py1, 0);

						 int px2 = PICTURE_BOX_SIZE;
						 int py2 = ((px2 - PICTURE_BOX_SIZE_DIV_TWO) * matrixHolder[0] + PICTURE_BOX_SIZE_DIV_TWO * matrixHolder[1] + matrixHolder[2] * pad) / (matrixHolder[1]);
